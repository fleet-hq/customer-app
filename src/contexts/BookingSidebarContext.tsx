...
export interface DateTimeState {
  ...
  minTime?: string | null;
  maxTime?: string | null;
  /** Friendly label like ``"Times shown in location's local time (America/Anchorage)"``. */
   ...
   *  unavailability blocks plus existing booking ranges. Only days
   *  with NO free hours show up here; days with partial availability
   *  stay selectable and their booked hours are surfaced via
   *  ``pickupDisabledSlots`` / ``dropoffDisabledSlots``. */
  unavailableDates?: string[];
  /** HH:mm slots disabled in the pickup time picker, given the
   *  currently-selected pickup date. Lets a 2-hour booking carve out
   *  just those 2 hours instead of the whole day. */
  pickupDisabledSlots?: string[];
  /** HH:mm slots disabled in the dropoff time picker. */
  dropoffDisabledSlots?: string[];
}
