...
export async function getAgreementByBookingId(bookingId: number | string): Promise<AgreementData | null> {
  try {
    const res = await axios.get<ApiAgreement[] | { results: ApiAgreement[] }>(
    ...
    // Backend disables pagination for non-admin (customer) requests,
    // so the response shape is a raw array. Admins still get the
    // standard ``{results: [...]}``. Handle both — without this, the
    // customer's saved signature wasn't rendering on re-open even
    // though the row was present in the DB.
...
// ``DefaultLocationInfo`` replaces the legacy ``DefaultBranchInfo``
// shape as part of the Branch → Location migration; the field
// names mirror the flat-Locations columns on the backend
// (``opening_time`` / ``closing_time`` not ``operational_*``).
export interface DefaultLocationInfo {
  id: number;
  name: string;
  address: string | null;
  ...
  timezone: string | null;
  is247: boolean;
  /** ``HH:mm:ss`` — location opening time, or null when unset. */
  openingTime: string | null;
  /** ``HH:mm:ss`` — location closing time, or null when unset. */
...
export interface CompanySettings {
  ...
  phone: string;
  logo: string | null;
  /** Company's default operational location — single source of
   *  truth for the customer-facing time pickers (timezone +
   *  opening hours). Replaces the legacy ``defaultBranch``
   *  field as part of the Branch → Location migration. */
  defaultLocation: DefaultLocationInfo | null;
}

interface ApiDefaultLocation {
  id?: number;
  name?: string;
  address?: string | null;
  timezone?: string | null;
...
function transformDefaultLocation(
  api?: ApiDefaultLocation | null,
...
export async function getCompanySettings(): Promise<CompanySettings> {
  try {
    ...
    const res = await axios.get<{
      ...
      phone_no?: string;
      company_picture?: string | null;
      default_location?: ApiDefaultLocation | null;
    }>(
    ...
    return {
      ...
      logo: company.company_picture || null,
      defaultLocation: transformDefaultLocation(company.default_location),
    };
  } catch {
